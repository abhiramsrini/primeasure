...
$fromAddress = trim((string)($config['content_request_from_address'] ?? $config['event_from_address'] ?? $config['from_address'] ?? ''));
$toAddress = trim((string)($config['content_request_to_address'] ?? $config['event_to_address'] ?? $config['to_address'] ?? ''));
$ccAddress = trim((string)($config['content_request_cc_address'] ?? $config['event_cc_address'] ?? $config['cc_address'] ?? ''));
...
if (str_len($name) < 2 || str_len($name) > 100) {
    json_response(['success' => false, 'message' => 'Please enter a valid full name.'], 400);
}
...
if (str_len($company) < 2 || str_len($company) > 120) {
    json_response(['success' => false, 'message' => 'Please enter a valid company name.'], 400);
}
...
if ($accessToken === null) {
    json_response(['success' => false, 'message' => 'Unable to submit your request right now.'], 502);
}

$mailPayload = [
    'fromAddress' => $fromAddress,
    'toAddress' => normalize_address_list($toAddress),
    'subject' => 'Event Content Request: ' . $eventTitle,
    'content' => build_message_body($eventTitle, $eventType, $eventId, $name, $email, $company, $ipAddress),
...
$ackEnabled = config_bool($config['content_request_ack_enabled'] ?? $config['event_ack_enabled'] ?? true);
if ($ackEnabled) {
    $ackFromAddress = trim((string)($config['content_request_ack_from_address'] ?? $config['event_ack_from_address'] ?? $fromAddress));
    if ($ackFromAddress !== '' && !is_placeholder_value($ackFromAddress)) {
        $ackSubjectTemplate = trim((string)($config['content_request_ack_subject'] ?? 'Content Request Received - {{event_title}}'));
        $ackSubject = str_replace('{{event_title}}', $eventTitle, $ackSubjectTemplate !== '' ? $ackSubjectTemplate : 'Content Request Received - {{event_title}}');
        ...
        if (!$ackResult['success']) {
            error_log('Content request acknowledgement email failed to send.');
        }
    }
}
...
function load_config(): array
{
    ...
    return [
        ...
        'content_request_from_address' => getenv('CONTENT_REQUEST_FROM_ADDRESS') ?: '',
        'content_request_to_address' => getenv('CONTENT_REQUEST_TO_ADDRESS') ?: '',
        'content_request_cc_address' => getenv('CONTENT_REQUEST_CC_ADDRESS') ?: '',
        'content_request_ack_enabled' => getenv('CONTENT_REQUEST_ACK_ENABLED') ?: '1',
        'content_request_ack_from_address' => getenv('CONTENT_REQUEST_ACK_FROM_ADDRESS') ?: '',
        'content_request_ack_subject' => getenv('CONTENT_REQUEST_ACK_SUBJECT') ?: 'Content Request Received - {{event_title}}',
...
function enforce_rate_limit(string $ipAddress, int $windowSeconds, int $maxRequests): bool
{
    ...
    $key = hash('sha256', $ipAddress . '|content-request');
    $file = $dir . '/' . $key . '.json';
    $now = time();
    $timestamps = [];
...
function build_message_body(
    ...
    $lines = [
        'New event content request from primeasure.com',
        '',
        'Event: ' . $eventTitle,
...
function build_acknowledgement_body(string $name, string $eventTitle): string
{
    ...
    $lines = [
        'Dear ' . $recipient . ',',
        '',
        'Thank you for requesting content for "' . $eventTitle . '" with Primeasure Technologies.',
        'We have received your request successfully.',
        '',
        'Our team will review your request and share available content shortly.',
