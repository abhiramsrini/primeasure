...
- Key assets: `index.html`, section pages under `about/`, `solutions/`, `contact/`, `events/`, `register/`, `request-content/`, `blog/`, shared styles in `css/`, scripts in `js/`, data in `data/`, templates in `templates/`, form APIs in `api/`.
...
- Forms: maintain validation/submission flows in `js/FormValidator.js` and `js/ContentRequestForm.js`; do not expose or log submitted data.
